...
<form action="${pageContext.request.contextPath}/CarrDelayCntServlet" method="post" name="f">
  <select name="year">
  ...
  <input type="radio" name="kbn" value="d"
     <c:if test="${param.kbn=='d'}">checked="checked"</c:if>>출발지연&nbsp;<br>
      ...
      var config = {
    	  ...
    	  data : {
    	     datasets : [{
                ...
                backgroundColor :
                <c:if test="${param.graph=='bar'}">	
                	[<c:forEach items="${map}" var="m">rcolor,</c:forEach>]
                </c:if>	
                    <c:if test="${param.graph=='pie'}">	
