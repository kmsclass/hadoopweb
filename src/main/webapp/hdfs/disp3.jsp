...
<%
	Path pt = new Path(path +"/"+ file); //읽을 파일 path 정보
	Configuration conf = new Configuration();
	FileSystem fs = FileSystem.get(conf);  
	//fs.open(pt) : pt가 지정하는 파일 열기. 
	BufferedReader br = new BufferedReader(new InputStreamReader(fs.open(pt),"UTF-8"));
%>
<h3><%=path +"/"+ file %>내용보기</h3>
<% int cnt = 0;
   if(start == null || start.equals("")) { //start 파라미터가 없는 경우
	   cnt = 1;
   } else {
	   cnt = Integer.parseInt(start);
   }
   for(int a=1;a<cnt;a++) br.readLine(); //cnt 만큼 skip함. 
   int i = 0;
   String line = null;
   while((line= br.readLine()) != null && i < 10){ //10줄씩 읽어서 화면에 출력하기
...
<a href="disp1.jsp">처음페이지로 이동</a></body></html>